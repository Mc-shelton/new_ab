...
import { roomsService } from "../../handlers/services";
...
const socketSetup = (server: any) => {
  ...
  function broadCastMessage(ws: WebSocket, group: any, data: any) {
    // ws.broadcast.emit
    console.log("broadcasting to group", data.group);
    ...
    ws.on("message", async (message: any) => {
      try {
        ...
        if (data.type === "message" && data.group && data.content) {
          ...
          broadCastMessage(ws, data.group, broadcastPayload);
