...
const getLessons = async (req: IRequest, res: Response) => {
    try {
        ...
        const b_data = books.data.filter((l:any) => {
...
const getPioneer = async (req: IRequest, res: Response) => {
    try {
        // Make the request to the remote resource
        const response = await axios.get(`${base_urls.e}tree/treedata.php?parentnode=f_16`);
...
const getPioneerEstate = async (req: IRequest, res: Response) => {
    ...
    try {
        // Make the request to the remote resource
        const response = await axios.get(`${base_urls.e}tree/treedata.php?parentnode=${node}`);
        // Send only the response data to the client
        res.status(200).json({
            message: 'estate media',
            data: response.data,
...
const getEgwFolder = async (req: IRequest, res: Response) => {
    ...
    try {
        // Make the request to the remote resource
        const response = await axios.get(`${base_urls.e}tree/treedata.php?parentnode=${url}`);
...
const getEgwContent = async (req: IRequest, res: Response) => {
    ...
    try {
        ...
        // Make the request to the remote resource
        const response = await axios.get(`${base_urls.e}textview/getid_elementauto.php?id_pub=${id_pub}`);
        ...
        const content = await axios.get(`${base_urls.e}writingsview/showrecords.php?startID=${id_el}&amount=${response.data.maxpuborder.toString()}&initload=true&mobile=0`);
